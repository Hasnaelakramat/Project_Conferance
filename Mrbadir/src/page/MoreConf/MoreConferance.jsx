...
const MoreConferance  = () => {
  const { conferenceId } = useParams();
  const idc= conferenceId;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/api/conferences/${idc}`);
...
<div className="container" >
        ...
        spacing={2}
         >
               <Button
                      variant="contained"
                      sx={{ backgroundColor: '#0F52BA', color: '#FFFFFF' }} // Couleur de fond et de texte
